<?php  if (!defined('BASEPATH')) exit('No direct script public allowed');
/**
 * CodeIgniter
 *
...
class CI_Template {
    ...
    *
    * @public  public
    * @param   string   array key to public template settings
    * @return  void
    */
   ...
   function add_js($script, $type = 'import', $position = 'footer',$defer = FALSE)
   {
      $success = TRUE;
      $js = NULL;
   ...
   /**  Set Breadcrumb */
   function set_breadcrumb($data = array()){
       if(sizeof($data) > 0 && isset($data)){
            $this->write_view("breadcrumb","breadcrumb",  array('breadcrumb'=>$data)); 
        }
   }
   /**
    * set_pagetitle 
   ...
   function set_pagetitle($title = ''){
       if(isset($title)){
            $this->write_view("page_title","page_title",  array('page_title'=>$title));
            $this->write("title",$title);
        }
   }
   /**
    * Add library data css and js data
    * @param type $type
    * @param type $position
    * @return boolean
    */
   function add_library($type = '', $position = 'footer') {
        $file = array();
        $css_file = array();
        $success = FALSE;
        // định nghĩa các library file
        switch ($type) {
                case 'jq_validate':
                    $file[] = 'lib/validation/jquery.validate.min.js';
                    $file[] = 'js/jquery_validate_messages.js';
                
                    break;
                case 'chosen':
                    $file[] = 'lib/chosen/chosen.jquery.min.js';
                // css file
                    $css_file[] = 'lib/chosen/chosen.css' ;
                    break;
                case  'datepicker':
                    $file[] = 'lib/datepicker/bootstrap-datepicker.min.js';
                    $file[] = 'lib/datepicker/bootstrap-timepicker.min.js';
                    $css_file[] = 'lib/datepicker/datepicker.css';
        ...
        // add js file
        $position_js = $position == 'top' ? 'js_top' : 'js_footer';
        if ($file != NULL && is_array($file)) {
            sort($file);
            foreach ($file as $file_js){
                $filepath = base_url() . 'public/' . $file_js;
                $js_lib = '<script type="text/javascript" src="' . $filepath . '"';
                $js_lib .= "></script>\n";  
                $this->write($position_js, $js_lib);
            }
            $success = TRUE;
        }
        // add css file
        if(sizeof($css_file) > 0){
            foreach ($css_file as $lib_css){
                $filepath = base_url() . 'public/' . $lib_css;                
                $css = '<link type="text/css" rel="stylesheet" href="'. $filepath .'"';
                $css .= ' />';
                $this->write('_styles', $css);
            }
 ...
 * 
 * @param type $ms
 * @param type $type error or info or warning
 */
    function  show_messages(){
        $this->CI->load->library('session');
        $ms =  $this->CI->session->flashdata('messages_data');
        $type = $this->CI->session->flashdata('messages_type');        
        if(isset($ms) && $ms !== FALSE){
            $this->write_view("messages_alert", "messages_alert", array('ms'=>$ms,'type'=>$type)); 
        }
    }
